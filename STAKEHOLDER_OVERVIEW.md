...
After a guest submits a free-text review, the system first uses a local ML model (MiniLM) to read the review and identify which of the 15 topics it already covers. Those topics are excluded from consideration. GPT-4o-mini then receives only the remaining gaps - so if a guest already mentioned the spa in their review, the system will not ask about the spa and instead targets the next most urgent uncovered topics. The AI writes 2 natural, conversational follow-up questions and decides the best response format for each - yes/no, free text, or multiple choice.
...
4. Submits, and immediately sees an **Impact** screen showing: how many points they earned, their current reviewer level and progress toward the next, which topic gaps they helped fill, and any perks unlocked at their level
